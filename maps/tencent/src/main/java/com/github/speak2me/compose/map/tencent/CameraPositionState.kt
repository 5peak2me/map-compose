 ...
 * A [CameraPositionState] may only be used by a single [TencentMap] composable at a time
...
public class CameraPositionState private constructor(
        ...
        set(value) {
            synchronized(lock) {
                ...
                if (map == null) {
                    ...
                    map.moveCamera(CameraUpdateFactory.newCameraPosition(value.toSdkPosition()))
                }
            }
        }
    ...
    // Guarded by `lock`.
    private var map: TencentMap? by mutableStateOf(null)
    ...
    internal fun setMap(map: TencentMap?) {
        synchronized(lock) {
            ...
            if (map == null) {
                ...
                map.moveCamera(CameraUpdateFactory.newCameraPosition(position.toSdkPosition()))
            }
            onMapChanged?.let {
    ...
    public companion object {
        ...
        public operator fun invoke(
            position: CameraPosition = CameraPosition(LatLng(39.984186, 116.307503), 13f, 0f, 0f)
...
public val currentCameraPositionState: CameraPositionState
    @[TencentMapComposable ReadOnlyComposable Composable]
