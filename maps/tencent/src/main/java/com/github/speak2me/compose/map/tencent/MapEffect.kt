 ...
 * managed [TencentMap] object into the composition's [CoroutineContext]. This effect will be
 * re-launched when a different [key1] is provided.
 *
 * Note: This effect should be used with caution as the [TencentMap]'s properties is managed by the
 * [com.github.speak2me.compose.map.tencent.TencentMap] composable function. However,
...
public fun MapEffect(key1: Any?, block: suspend CoroutineScope.(TencentMap) -> Unit) {
...
@TencentMapComposable
@MapsComposeExperimentalApi
public fun MapEffect(key1: Any?, key2: Any?, block: suspend CoroutineScope.(TencentMap) -> Unit) {
    val map = (currentComposer.applier as MapApplier).map
...
public fun MapEffect(
    ...
    key2: Any?,
    key3: Any?,
    block: suspend CoroutineScope.(TencentMap) -> Unit
) {
    val map = (currentComposer.applier as MapApplier).map
