...
import com.github.speak2me.compose.map.tencent.ktx.addGroundOverlay
...
//            update(clickable) { this.groundOverlay.isClickable = it }
            update(image) { this.groundOverlay.setBitmap(it) }
...
//        return position(position.location, position.width.toInt())
        return position(position.location)
    }

//    return position(position.location, position.width.toInt(), position.height.toInt())
    return position(position.location)
}
