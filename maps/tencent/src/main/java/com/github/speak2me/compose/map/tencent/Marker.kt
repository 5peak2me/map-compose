...
public class MarkerState private constructor(position: LatLng) {
     ...
     * [com.tencent.tencentmap.mapsdk.maps.TencentMap] special semantics:
     * only a single info window can be visible for the entire TencentMap.
     *
...
private fun MarkerImpl(
    ...
    ComposeNode<MarkerNode, MapApplier>(
        ...
        update = {
            ...
            update(flat) { this.marker.options.flat(it) }
...
//    pinConfig: PinConfig? = null,
    iconView: View? = null,
    collisionBehavior: MarkerCollisionRelation = MarkerCollisionRelation.ALONE
) {
    AdvancedMarkerImpl(
        state = state,
        contentDescription = contentDescription,
    ...
    ComposeNode<MarkerNode, MapApplier>(
        factory = {
            ...
            val advancedMarkerOptions = AdvancedMarkerOptions()
                .position(state.position)
                .setCollisionRelation(collisionBehavior)
