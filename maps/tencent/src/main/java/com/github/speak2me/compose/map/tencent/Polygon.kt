...
import com.github.speak2me.compose.map.tencent.ktx.addPolygon
...
public fun Polygon(
    ...
    strokeJointType: Int = PolylineOptions.LineType.LINE_TYPE_IMAGEINARYLINE,
...
//            update(geodesic) { this.polygon.isGeodesic = it }
            update(holes) { this.polygon.setHolePoints(it) }
            update(strokeColor) { this.polygon.strokeColor = it.toArgb() }
//            update(strokeJointType) { this.polygon.strokeJointType = it }
            update(strokePattern) { this.polygon.setOptions(polygonOptions { pattern(it) }) }
