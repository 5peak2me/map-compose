 ...
 *
 * @param modifier Modifier to be applied to the TencentMap
 ...
 * @param tencentMapOptionsFactory the block for creating the [TencentMapOptions] provided when the
 * map is created
 ...
 */
@Composable
public fun TencentMap(
    modifier: Modifier = Modifier,
    mergeDescendants: Boolean = false,
    cameraPositionState: CameraPositionState = rememberCameraPositionState(),
    contentDescription: String? = null,
    tencentMapOptionsFactory: () -> TencentMapOptions = { TencentMapOptions() },
...
private fun CoroutineScope.launchSubcomposition(
    ...
    // Use [CoroutineStart.UNDISPATCHED] to kick off TencentMap loading immediately
...
public typealias TencentMapFactory = @Composable () -> Unit

/**
 * This method provides a factory pattern for TencentMap. It can typically be used in tests to provide a default Composable
 * of type TencentMapFactory.
 *
...
private class MapLifecycleEventObserver(private val mapView: MapView) : LifecycleEventObserver {
    ...
    private fun invokeEvent(event: Lifecycle.Event) {
        when (event) {
            Lifecycle.Event.ON_CREATE -> /*mapView.onCreate(Bundle())*/Unit
...
/**
 * Enum representing a 1-1 mapping to [com.tencent.tencentmap.mapsdk.maps.TencentMap.MAP_TYPE_NORMAL].
 *
 * This enum provides equivalent values to facilitate usage with [com.tencent.tencentmap.mapsdk.maps.TencentMap].
 *
...
public enum class ComposeMapColorScheme(public val value: Int) {
    LIGHT(TencentMap.MAP_TYPE_NORMAL),
