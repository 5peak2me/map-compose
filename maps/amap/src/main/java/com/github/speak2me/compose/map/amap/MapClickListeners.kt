...
/**
 * @param L AMap click listener type, e.g. [OnMapClickListener]
 */
internal class MapClickListenerNode<L : Any>(
    private val map: AMap,
    private val setter: AMap.(L?) -> Unit,
    ...
    with(mapClickListeners) {
        ::indoorStateChangeListener.let { callback ->
            MapClickListenerComposeNode(
                ...
                object : OnIndoorBuildingActiveListener {
...
//                        callback().onIndoorBuildingFocused()
        ...
        ::onMapLoaded.let { callback ->
            MapClickListenerComposeNode(
                ...
                OnMapLoadedListener { callback()?.invoke() }
            )
        }
 ...
 * @param setter a reference to a AMap setter method, e.g. `setOnMapClickListener()`
...
private fun MapClickListenerComposeNode(
    ...
    // Setting a AMap listener may have side effects, so we unset it as needed.
    ...
    // when callbacks recompose rapidly; setting AMap listeners could potentially be
    // expensive due to synchronization, etc. AMap listeners are not designed with a
