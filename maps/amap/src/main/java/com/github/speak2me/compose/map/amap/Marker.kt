...
public class MarkerState private constructor(position: LatLng) {
     ...
     * [com.amap.api.maps.AMap] special semantics:
     * only a single info window can be visible for the entire AMap.
     *
 ...
 * [com.github.speak2me.compose.map.amap.Marker].
 *
 * @param state the [MarkerState] to be used to control or observe the marker
 * state such as its position and info window
 * @param alpha the alpha (opacity) of the marker
...
//                contentDescription(contentDescription)
        ...
        update = {
            ...
            update(tag) { this.marker.`object` = it }
            update(title) {
