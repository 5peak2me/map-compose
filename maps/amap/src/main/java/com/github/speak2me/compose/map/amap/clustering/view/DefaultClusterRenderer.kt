 ...
 */
public open class DefaultClusterRenderer<T : ClusterItem> @JvmOverloads constructor(
    context: Context,
...
//        mClusterManager.clusterMarkerCollection.setOnInfoWindowLongClickListener(null)
    ...
    private inner class RenderTask(
        ...
        @SuppressLint("NewApi")
        override fun run() {
            if (!shouldRender(immutableOf(this@DefaultClusterRenderer.mClusters), immutableOf(clusters))) {
                mCallback!!.run()
                return
            }
            ...
            if (this@DefaultClusterRenderer.mClusters != null && animate) {
                existingClustersOnScreen = ArrayList()
                for (c in this@DefaultClusterRenderer.mClusters!!) {
            ...
            mMarkers = newMarkers
            this@DefaultClusterRenderer.mClusters = clusters
            mZoom = zoom
