...
import com.github.speak2me.compose.map.amap.clustering.android.data.RendererLogger
 ...
 */
public open class ClusterRendererMultipleItems<T : ClusterItem> @JvmOverloads constructor(
    context: Context,
     ...
     */
    public var minClusterSize: Int = 2
...
//        mClusterManager.clusterMarkerCollection.setOnInfoWindowLongClickListener(null)
                ...
                if (onScreen && animate) {
                    ...
                    if (closest != null) {
                        ...
                        RendererLogger.d("ClusterRenderer", "Animating then removing marker at position: " + marker.position)
                            ...
                            .iterator()
                            .hasNext() &&
                        mClusterMarkerCache.mCache.keys
                            .iterator()
                            .next()
                            .items
                            .contains(marker.clusterItem)
                    ) {
                        ...
                        RendererLogger.d(
                            "ClusterRenderer",
                            "Animating then removing marker joining cluster at position: " + marker.position,
                        )
                    } else {
     ...
     */
    protected fun onClusterRendered(
        cluster: Cluster<T>,
        marker: Marker,
    ) {}
     ...
     */
    protected fun onClusterItemRendered(
        clusterItem: T,
        marker: Marker,
    ) {}
     ...
     */
    private class MarkerWithPosition<T>(
        val marker: Marker,
        val clusterItem: T?,
    ) {
        var position: LatLng? = marker.position

        override fun equals(other: Any?): Boolean = if (other is MarkerWithPosition<*>) {
            marker == other.marker
        } else {
            false
        }
