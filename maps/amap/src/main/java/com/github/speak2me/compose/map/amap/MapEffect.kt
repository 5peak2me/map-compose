 ...
 * managed [AMap] object into the composition's [CoroutineContext]. This effect will be
 * re-launched when a different [key1] is provided.
 *
 * Note: This effect should be used with caution as the [AMap]'s properties is managed by the
 * [com.github.speak2me.compose.map.amap.AMap] composable function. However,
...
public fun MapEffect(key1: Any?, block: suspend CoroutineScope.(AMap) -> Unit) {
...
@AMapComposable
@MapsComposeExperimentalApi
public fun MapEffect(key1: Any?, key2: Any?, block: suspend CoroutineScope.(AMap) -> Unit) {
    val map = (currentComposer.applier as MapApplier).map
...
public fun MapEffect(
    ...
    key2: Any?,
    key3: Any?,
    block: suspend CoroutineScope.(AMap) -> Unit
) {
    val map = (currentComposer.applier as MapApplier).map
