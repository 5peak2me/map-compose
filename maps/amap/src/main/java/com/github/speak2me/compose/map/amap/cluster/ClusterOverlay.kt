...
import com.github.speak2me.compose.map.amap.MapNode
...
// https://github.com/amap-demo/android-cluster-marker/blob/master/app/src/main/java/com/amap/apis/cluster/ClusterOverlay.java
// https://github.com/TheMelody/OmniMap-Compose/blob/master/gd-map-compose/src/main/java/com/melody/map/gd_compose/kernel/KernelClusterOverlay.kt
private class ClusterOverlay(
    ...
    private var mPXInMeters: Float = map.scalePerPixel
    ...
    init {
        ...
        mLruCache = LruCache<Int, BitmapDescriptor>(defaultCacheSize)
    ...
    override fun onMarkerClick(marker: Marker?): Boolean {
        ...
        if (cluster != null) {
            mClusterClickListener?.onClick(marker, cluster.clusterItems)
            return true
        }
        return false
    }
    ...
    private fun addSingleClusterToMap(cluster: Cluster) {
        ...
        val marker: Marker = map.addMarker(markerOptions {
            anchor(0.5f, 0.5f)
                .icon(getBitmapDescriptor(cluster.clusterCount))
                .position(latLng)
        })
    ...
    private fun getCluster(latLng: LatLng, clusters: List<Cluster>): Cluster? {
        for (cluster in clusters) {
            val distance = latLng.sphericalDistance(cluster.centerLatLng)
            if (distance < mClusterDistance && map.cameraPosition.zoom < 19) {
                return cluster
            }
        }
        return null
     ...
     * 获取每个聚合点的绘制样式
     */
    private fun getBitmapDescriptor(num: Int): BitmapDescriptor? {
        ...
        if (bitmapDescriptor == null) {
            ...
            if (mClusterRender != null && mClusterRender?.getDrawable(num) != null) {
                ...
                textView.background = defaultClusterIcon.bitmap.toDrawable(context.resources)
            }
    ...
    private fun updateCluster(cluster: Cluster) {
        cluster.marker?.setIcon(getBitmapDescriptor(cluster.clusterCount))
    }
     ...
     */
    private class MyAnimationListener(private val mRemoveMarkers: MutableList<Marker>) :
...
private class ClusterOverlayNode(
    val clusterOverlay: ClusterOverlay,
    var onClusterOverlayClick: (ClusterOverlay) -> Unit
) : MapNode {
...
public fun ClusterOverlay(
    ...
    ComposeNode<ClusterOverlayNode, MapApplier>(
        ...
        update = {
            update(clusterItems) { this.clusterOverlay.setClusterItems(it) }
            update(clusterRenderer) { this.clusterOverlay.setClusterRenderer(it) }
        }
    )
}