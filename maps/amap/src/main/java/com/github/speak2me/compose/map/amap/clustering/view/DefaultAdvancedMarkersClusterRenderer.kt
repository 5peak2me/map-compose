...
import com.github.speak2me.compose.map.amap.clustering.ClusterManager
...
import com.github.speak2me.compose.map.amap.clustering.android.ui.IconGenerator
import com.github.speak2me.compose.map.amap.clustering.android.ui.SquareTextView
 ...
 */
public open class DefaultAdvancedMarkersClusterRenderer<T : ClusterItem> @JvmOverloads constructor(
    context: Context,
     ...
     */
    public var minClusterSize: Int = 4
...
//        mClusterManager.clusterMarkerCollection.setOnInfoWindowLongClickListener(null)
    ...
    protected open fun onBeforeClusterItemRendered(
        item: T,
        advancedMarkerOptions: MarkerOptions,
    ) {
     ...
     */
    public fun getMarker(clusterItem: T): Marker? = mMarkerCache[clusterItem]
     ...
     */
    public fun getClusterItem(marker: Marker): T? = mMarkerCache[marker]
     ...
     */
    public fun getMarker(cluster: Cluster<T>): Marker? = mClusterMarkerCache[cluster]
     ...
     */
    public fun getCluster(marker: Marker): Cluster<T>? = mClusterMarkerCache[marker]
        ...
        fun perform(markerModifier: MarkerModifier) {
            ...
            if (!shouldRenderAsCluster(cluster)) {
                for (item in cluster.items) {
                    var marker = mMarkerCache[item] as Marker?
                    val markerWithPosition: MarkerWithPosition
                    if (marker == null) {
                        val advancedMarkerOptions = MarkerOptions()
                        if (animateFrom != null) {
                        ...
                        val newMarker = mClusterManager.markerCollection.addMarker(advancedMarkerOptions) as Marker
                        marker = newMarker
            ...
            var marker = mClusterMarkerCache[cluster] as Marker?
            val markerWithPosition: MarkerWithPosition
            if (marker == null) {
                val advancedMarkerOptions = MarkerOptions().position(animateFrom ?: cluster.position)
                onBeforeClusterRendered(cluster, advancedMarkerOptions)
                val newMarker = mClusterManager.clusterMarkerCollection.addMarker(advancedMarkerOptions) as Marker
                marker = newMarker
