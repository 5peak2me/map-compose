...
//        set(mapProperties.latLngBoundsForCameraTarget) { map.setLatLngBoundsForCameraTarget(it) }
        set(mapProperties.mapStyleOptions) {
            mapView.setMapCustomStyle(it, object : CustomMapStyleCallBack {
                override fun onPreLoadLastCustomMapStyle(customStylePath: String): Boolean = false

                override fun onCustomMapStyleLoadSuccess(hasUpdate: Boolean, customStylePath: String): Boolean = false

                override fun onCustomMapStyleLoadFailed(status: Int, message: String, customStylePath: String): Boolean = false
            })
...
//        set(mapUiSettings.scrollGesturesEnabledDuringRotateOrZoom) { map.uiSettings.isScrollGesturesEnabledDuringRotateOrZoom = it }
        set(mapUiSettings.tiltGesturesEnabled) { map.uiSettings.isOverlookingGesturesEnabled = it }
...
private fun MapPropertiesNode.applyContentPadding(map: BaiduMap, contentPadding: PaddingValues) {
    val node = this
    with (this.density) {
        map.setViewPadding(
