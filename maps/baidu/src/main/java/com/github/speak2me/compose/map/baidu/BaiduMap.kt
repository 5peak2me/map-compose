...
import com.github.speak2me.compose.map.baidu.ktx.awaitMap
 ...
 *
 * @param modifier Modifier to be applied to the BaiduMap
 ...
 * @param baiduMapOptionsFactory the block for creating the [BaiduMapOptions] provided when the
 * map is created
...
public fun BaiduMap(
    ...
    baiduMapOptionsFactory: () -> BaiduMapOptions = { BaiduMapOptions() },
    ...
    content: @Composable @BaiduMapComposable () -> Unit = {},
) {
    ...
    AndroidView(
        modifier = modifier,
        factory = { context ->
            MapView(context, baiduMapOptionsFactory()).also { mapView ->
...
private fun CoroutineScope.launchSubcomposition(
    ...
    // Use [CoroutineStart.UNDISPATCHED] to kick off BaiduMap loading immediately
...
public typealias BaiduMapFactory = @Composable () -> Unit

/**
 * This method provides a factory pattern for BaiduMap. It can typically be used in tests to provide a default Composable
 * of type BaiduMapFactory.
 *
...
private class MapLifecycleEventObserver(private val mapView: MapView) : LifecycleEventObserver {
    ...
    private fun invokeEvent(event: Lifecycle.Event) {
        when (event) {
            Lifecycle.Event.ON_CREATE -> mapView.onCreate(mapView.context, Bundle())
            Lifecycle.Event.ON_START -> /*mapView.onStart()*/Unit
            Lifecycle.Event.ON_RESUME -> mapView.onResume()
            Lifecycle.Event.ON_PAUSE -> mapView.onPause()
            Lifecycle.Event.ON_STOP -> /*mapView.onStop()*/Unit
...
/**
 * Enum representing a 1-1 mapping to [BaiduMap.MAP_TYPE_NORMAL].
 *
 * This enum provides equivalent values to facilitate usage with [BaiduMap].
 *
...
public enum class ComposeMapColorScheme(public val value: Int) {
    LIGHT(BaiduMap.MAP_TYPE_NORMAL),
    DARK(BaiduMap.MAP_TYPE_SATELLITE),
}
