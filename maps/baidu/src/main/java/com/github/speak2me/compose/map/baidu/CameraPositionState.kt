 ...
 * A [CameraPositionState] may only be used by a single [BaiduMap] composable at a time
...
public class CameraPositionState private constructor(
    position: MapStatus = MapStatus.Builder().target(LatLng(0.0, 0.0)).build()
) {
    /**
        ...
        set(value) {
            synchronized(lock) {
                ...
                if (map == null) {
                    ...
                    map.animateMapStatus(MapStatusUpdateFactory.newMapStatus(value))
                }
            }
        }
    ...
    // Guarded by `lock`.
    private var map: BaiduMap? by mutableStateOf(null)
    ...
    internal fun setMap(map: BaiduMap?) {
        synchronized(lock) {
            ...
            if (map == null) {
                ...
                map.animateMapStatus(MapStatusUpdateFactory.newMapStatus(position))
            }
            onMapChanged?.let {
     ...
     *
     * If this [CameraPositionState] is not currently bound to a [BaiduMap] this call will
     ...
     */
    @UiThread
    public suspend fun animate(update: MapStatusUpdate, durationMs: Int = MAX_VALUE) {
                        ...
                        val animateOnMapAvailable = object : OnMapChangedCallback {
                            override fun onMapChangedLocked(newMap: BaiduMap?) {
                                if (newMap == null) {
                                    ...
                                    error(
                                        "internal error; no BaiduMap available to animate position"
                                    )
                                }
...
//        val cancelableCallback = object : BaiduMap.CancelableCallback {
...
//            map.animateCamera(update, durationMs.toLong(), cancelableCallback)
        ...
        doOnMapChangedLocked {
            check(it == null) {
                "New BaiduMap unexpectedly set while an animation was still running"
            }
//            map.stopAnimation()
        }
    }
     ...
     */
    @UiThread
    public fun move(update: MapStatusUpdate) {
        synchronized(lock) {
            val map = map
            movementOwner = null
            if (map == null) {
                // Do it when we have a map available
                doOnMapChangedLocked { it?.animateMapStatus(update) }
            } else {
                map.animateMapStatus(update)
            }
        }
...
//            position: CameraPosition = CameraPosition(LatLng(0.0, 0.0), 0f, 0f, 0f)
         ...
         */
        public val Saver: Saver<CameraPositionState, MapStatus> = Saver(
            save = { it.position },
...
public val currentCameraPositionState: CameraPositionState
    @[BaiduMapComposable ReadOnlyComposable Composable]
