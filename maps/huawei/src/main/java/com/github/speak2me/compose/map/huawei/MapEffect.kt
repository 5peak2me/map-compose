 ...
 * managed [HuaweiMap] object into the composition's [CoroutineContext]. This effect will be
...
public fun MapEffect(key1: Any?, block: suspend CoroutineScope.(HuaweiMap) -> Unit) {
...
@HuaweiMapComposable
@MapsComposeExperimentalApi
public fun MapEffect(key1: Any?, key2: Any?, block: suspend CoroutineScope.(HuaweiMap) -> Unit) {
    val map = (currentComposer.applier as MapApplier).map
...
public fun MapEffect(
    ...
    key2: Any?,
    key3: Any?,
    block: suspend CoroutineScope.(HuaweiMap) -> Unit
) {
    val map = (currentComposer.applier as MapApplier).map
