 ...
 * A [CameraPositionState] may only be used by a single [HuaweiMap] composable at a time
...
public class CameraPositionState private constructor(
    ...
    // Guarded by `lock`.
    private var map: HuaweiMap? by mutableStateOf(null)
    ...
    private fun interface OnMapChangedCallback {
        fun onMapChangedLocked(newMap: HuaweiMap?)
        fun onCancelLocked() {}
    }

    // The current map is set and cleared by side effect.
    // There can be only one associated at a time.
    internal fun setMap(map: HuaweiMap?) {
        synchronized(lock) {
            if (this.map == null && map == null) return
            if (this.map != null && map != null) {
                error("CameraPositionState may only be associated with one HuaweiMap at a time")
            }
            this.map = map
            if (map == null) {
                isMoving = false
     ...
     *
     * If this [CameraPositionState] is not currently bound to a [HuaweiMap] this call will
                ...
                synchronized(lock) {
                    ...
                    if (map == null) {
                        ...
                        val animateOnMapAvailable = object : OnMapChangedCallback {
                            override fun onMapChangedLocked(newMap: HuaweiMap?) {
                                if (newMap == null) {
                                    // Cancel the animate caller and crash the map setter
                                    @Suppress("ThrowableNotThrown")
                                    continuation.resumeWithException(CancellationException(
                                            "internal error; no HuaweiMap available"))
                                    error(
                                        "internal error; no HuaweiMap available to animate position"
                                    )
                                }
    ...
    private fun performAnimateCameraLocked(
        ...
        doOnMapChangedLocked {
            check(it == null) {
                "New HuaweiMap unexpectedly set while an animation was still running"
            }
            map.stopAnimation()
        }
    }
...
public val currentCameraPositionState: CameraPositionState
    @[HuaweiMapComposable ReadOnlyComposable Composable]
