...
internal object ElfReader {
    ...
    fun parseHeader(buf: ByteArray): ElfInfo? {
        ...
        val is64bits = (cls == 2) // 1: 32 bits, 2: 64 bits
        return if (is64bits) {
    ...
    private fun readU16(buf: ByteArray, offset: Int, order: ByteOrder): Int {
        val bb = ByteBuffer.wrap(buf, offset, 2).order(order)
        return bb.short.toInt() and 0xffff
    }

    private fun readU32(buf: ByteArray, offset: Int, order: ByteOrder): Int {
        val bb = ByteBuffer.wrap(buf, offset, 4).order(order)
        return bb.int
    }

    private fun readU64(buf: ByteArray, offset: Int, order: ByteOrder): Long {
